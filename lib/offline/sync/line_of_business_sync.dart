...
class LineOfBusinessSync extends BaseSync<LineOfBusinessModel> {
  ...
  Future<List<dynamic>> getAll<T extends BaseModel>() async {
    List<MasterDataDBModel> result = await DatabaseHelper().getAllById(
        MasterDataDBModel(),
        MasterTable.LineOfBusiness.toString(),
        'masterDataName');

    List<LineOfBusinessModel> returnResult = List();
    result.forEach((f) =>
        returnResult.add(LineOfBusinessModel.fromJson(decode(f.jsonData))));
    return returnResult;
  }
  ...
  Future<List> getApiData<T extends BaseModel>(
    ...
    return await _lineOfBusinessApi.getAllLineOfBusiness();
  }
}
